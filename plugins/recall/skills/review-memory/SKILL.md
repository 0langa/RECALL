...
2. Add `--status`, `--category`, or `--source` filters when the user asks about a specific slice.
3. Use returned IDs for lifecycle operations such as `confirm-memory`, `resolve-memory`, `stale-memory`, `supersede-memory`, `merge-memories`, or `prune-memory`.
4. Treat the review as evidence to inspect, not as guaranteed truth over the current repository state.

## Result Handling

Report counts and the most relevant IDs. Prefer concise summaries over dumping every record. If many low-value automatic memories are present, recommend non-destructive archival with `prune-memory` rather than deletion.
